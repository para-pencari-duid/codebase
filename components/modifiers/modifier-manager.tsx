...
import {
    Plus, Pencil, Trash2, GripVertical, ListPlus, Package,
} from "lucide-react";
...
export function ModifierManager({ initialGroups, availableItems }: ModifierManagerProps) {
    ...
    const addOption = () => {
        setFormOptions([...formOptions, { name: "", price: 0, isActive: true, sortOrder: formOptions.length }]);
    };
    ...
    const updateOption = (idx: number, field: keyof ModifierOption, value: any) => {
    ...
    const handleSave = async () => {
        ...
        if (validOptions.length === 0) { toast.error("Minimal 1 option"); return; }
    ...
    const handleDelete = async (groupId: string) => {
        ...
        try {
            const res = await fetch(`/api/modifiers/${groupId}`, { method: "DELETE" });
        ...
        <>
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm">Kelola modifier / add-on untuk produk (topping, level gula, ukuran, dll.)</p>
                </div>
                ...
                <Card className="border-dashed">
                    <CardContent className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm">Buat modifier group pertama untuk mulai menambahkan add-on ke produk.</p>
                        </div>
                        <Button variant="outline" onClick={openCreate} className="gap-1.5 mt-2">
                            <Plus className="h-4 w-4" />
                            Tambah Group
                        </Button>
                    </CardContent>
                                ...
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <CardDescription className="mt-1 flex gap-1.5 flex-wrap">
                                            {group.required && <Badge variant="destructive" className="text-[10px]">Wajib</Badge>}
                                            <Badge variant="outline" className="text-[10px]">
                                                {group.multiple ? "Multi-pilih" : "Pilih 1"}
                                            </Badge>
                                            {group.maxSelect && (
                                                <Badge variant="outline" className="text-[10px]">Maks. {group.maxSelect}</Badge>
                                            )}
                                        </CardDescription>
                                    </div>
                            ...
                            <CardContent className="pt-0 space-y-3">
                                <div className="space-y-1">
                                    {group.options.map((opt) => (
                                        <div key={opt.id || opt.name} className="flex justify-between text-sm py-1 px-2 bg-slate-50 rounded">
                                            <span>{opt.name}</span>
                                            {opt.price > 0 && (
                                    ...
                                    <>
                                        ...
                                        <div className="flex items-center gap-1 flex-wrap">
                                            ...
                                            {group.items.slice(0, 3).map((it) => (
                                                <Badge key={it.id} variant="secondary" className="text-[10px]">{it.name}</Badge>
                                            ))}
                                            {group.items.length > 3 && (
                                                <span className="text-[10px] text-muted-foreground">+{group.items.length - 3} lainnya</span>
                                            )}
                                        </div>
                                    </>
                                )}
            ...
            <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o) { setDialogOpen(false); resetForm(); } }}>
                <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editingGroup ? "Edit Modifier Group" : "Tambah Modifier Group"}</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="flex gap-6">
                            <div className="flex items-center gap-2">
                                <Switch checked={formRequired} onCheckedChange={setFormRequired} id="required" />
                                <Label htmlFor="required" className="text-sm">Wajib dipilih</Label>
                            </div>
                            <div className="flex items-center gap-2">
                                <Switch checked={formMultiple} onCheckedChange={setFormMultiple} id="multiple" />
                                <Label htmlFor="multiple" className="text-sm">Multi-pilih</Label>
                            </div>
                        </div>
                        ...
                        {formMultiple && (
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    value={formMaxSelect}
                                    onChange={(e) => setFormMaxSelect(e.target.value ? parseInt(e.target.value) : "")}
                                />
                            </div>
                        )}
                            ...
                            <div className="max-h-40 overflow-y-auto border rounded-md p-2 space-y-1">
                                {availableItems.map((item) => (
                                    ...
                                    >
                                        ...
                                        />
                                        <span>{item.name}</span>
                                        <span className="text-xs text-muted-foreground ml-auto">{item.sku}</span>
                                    </label>
                                ))}
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleSave} disabled={loading}>
                            {loading ? "Menyimpan..." : editingGroup ? "Simpan Perubahan" : "Tambah"}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
