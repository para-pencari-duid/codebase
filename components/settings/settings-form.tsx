...
import { Save, Store, Receipt, Calculator, CreditCard, QrCode, Gift, Upload, CheckCircle2, XCircle, Loader2, ChevronDown } from "lucide-react";
...
export function SettingsForm({ initialData }: SettingsFormProps) {
    ...
    const [qrisDecodeStatus, setQrisDecodeStatus] = useState<"idle" | "decoding" | "success" | "error">("idle");
    ...
    const handleQrisImageUpload = useCallback(async (file: File) => {
        ...
        try {
            ...
            img.onerror = () => { URL.revokeObjectURL(objectUrl); setQrisDecodeStatus("error"); };
            img.src = objectUrl;
        } catch {
            setQrisDecodeStatus("error");
        }
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input placeholder="Nama Usaha Anda" {...field} disabled={loading} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input placeholder="info@usaha.com" {...field} disabled={loading} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Upload foto / screenshot QRIS statis Anda — sistem akan baca string QR otomatis dan inject nominal saat checkout
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div
                                ...
                                onDrop={(e) => {
                                    ...
                                    if (file && file.type.startsWith("image/")) handleQrisImageUpload(file);
                                }}
                            >
                                <input
                                ...
                                {qrisDecodeStatus === "decoding" && (
                                    <>
                                        ...
                                        <p className="text-sm text-muted-foreground">Membaca QR code dari gambar...</p>
                                    </>
                                )}
                                ...
                                {qrisDecodeStatus === "success" && (
                                    <>
                                        ...
                                        <div className="text-center">
                                            <p className="text-sm font-semibold text-green-700">QR code berhasil dibaca!</p>
                                            <p className="text-xs text-muted-foreground mt-0.5">{qrisFileName}</p>
                                        </div>
                                        <p className="text-xs text-muted-foreground">Klik untuk upload ulang</p>
                                    </>
                                )}
                                ...
                                {qrisDecodeStatus === "error" && (
                                    <>
                                        ...
                                        <div className="text-center">
                                            <p className="text-sm font-semibold text-destructive">Gagal membaca QR code</p>
                            ...
                            {form.watch("qrisString") && qrisDecodeStatus === "idle" && (
                                <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-800">
                                    ...
                                    QRIS aktif — nominal akan di-inject otomatis saat pembayaran QRIS di POS.
                                </div>
                            )}

                            {/* Manual input fallback */}
                            <Collapsible open={showManualQris} onOpenChange={setShowManualQris}>
                                <CollapsibleTrigger asChild>
                                    <button type="button" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
                                        <ChevronDown className={`h-3.5 w-3.5 transition-transform ${showManualQris ? "rotate-180" : ""}`} />
                                            ...
                                            <FormItem>
                                                <FormControl>
                                                    <Textarea
                                                        ...
                                                        onChange={(e) => {
                                                            field.onChange(e);
                                                            if (e.target.value) setQrisDecodeStatus("success");
                                                            else setQrisDecodeStatus("idle");
                                                        }}
                                                    />
                                                </FormControl>
                                                <FormDescription className="text-xs">
                                                    Copy isi teks dari QR code QRIS statis (bukan gambarnya). Bisa dari aplikasi bank dengan fitur "salin konten QR".
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Berikan poin reward kepada pelanggan setiap transaksi, bisa ditukar saat checkout
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                                        <div className="space-y-0.5">
                                            <FormLabel className="text-base">Aktifkan Program Loyalty</FormLabel>
                                            <FormDescription>
                                                Pelanggan mendapat poin setiap transaksi dan bisa redeem saat bayar
                                            </FormDescription>
                                        </div>
                                        <FormControl>
                                            <Switch
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormDescription>
                                                    Setiap berbelanja Rp{(form.watch("loyaltyPointsPerRupiah") || 1000).toLocaleString("id-ID")} pelanggan dapat 1 poin
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormDescription>
                                                    1 poin setara Rp{(form.watch("loyaltyPointValue") || 1).toLocaleString("id-ID")} saat redeem
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                            ...
                            {form.watch("loyaltyEnabled") && (
                                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                                    Contoh: Belanja Rp{(form.watch("loyaltyPointsPerRupiah") || 1000).toLocaleString("id-ID")} → 1 poin.
                                    1000 poin → Rp{((form.watch("loyaltyPointValue") || 1) * 1000).toLocaleString("id-ID")} diskon.
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Aktifkan modul tambahan sesuai kebutuhan bisnis Anda</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            {(
                                [
                                    ["tierPricingEnabled", "Harga Bertingkat (Tier Pricing)", "Harga khusus per segmen / tier pelanggan"],
                                    ["consignmentEnabled", "Konsinyasi", "Manajemen barang titipan dari supplier"],
                                    ["serialTrackEnabled", "Serial Number Tracking", "Lacak produk berdasarkan nomor seri / IMEI"],
                                    ["accountingEnabled", "Akuntansi (Double-Entry)", "Jurnal, chart of accounts, dan tarif pajak"],
                                    ["bankReconEnabled", "Rekonsiliasi Bank", "Cocokkan mutasi bank dengan transaksi POS"],
                                    ["payrollEnabled", "Penggajian Karyawan", "Absensi, slip gaji, dan rekap payroll"],
                                    ["marketingEnabled", "Marketing & Kampanye", "Kirim pesan WhatsApp/Email massal ke pelanggan"],
                                    ["feedbackEnabled", "Feedback & NPS", "Kumpulkan rating dan skor kepuasan pelanggan"],
                                    ["onlineOrderEnabled", "Order Online / Marketplace", "Integrasi dengan platform marketplace"],
                                    ["webhooksEnabled", "Webhook & API", "Kirim event real-time ke sistem eksternal"],
                                ] as const
                            ).map(([name, label, desc]) => (
                                <FormField
                                    key={name}
