...
const defaultForm = {
  ...
  baseSalary: "", bankName: "", bankAccount: "", bankHolder: "", joinDate: "",
};
...
export default function EmployeeClient() {
  ...
  const markAttendance = async (employeeId: string, status: "PRESENT" | "ABSENT") => {
    try {
      ...
      toast.success(`Absensi ${status === "PRESENT" ? "hadir" : "tidak hadir"} dicatat`);
    } catch {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {(["name","position","department","phone","bankName","bankAccount","bankHolder"] as const).map((field) => (
                <div key={field}>
          ...
          <TableBody>
            {employees.map((emp) => (
              <TableRow key={emp.id}>
                ...
                <TableCell>Rp {emp.baseSalary.toLocaleString("id-ID")}</TableCell>
                <TableCell>{emp.joinDate ? new Date(emp.joinDate).toLocaleDateString("id-ID") : "-"}</TableCell>
                <TableCell>
                  <Badge className={emp.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}>
                ...
                <TableCell>
                  <div className="flex gap-1">
                    <Button size="sm" variant="outline" className="text-green-700 border-green-300"
                      onClick={() => markAttendance(emp.id, "PRESENT")}>Hadir</Button>
                    <Button size="sm" variant="outline" className="text-red-700 border-red-300"
                      onClick={() => markAttendance(emp.id, "ABSENT")}>Absen</Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
            {employees.length === 0 && (
              <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground">Belum ada karyawan</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </CardContent>
